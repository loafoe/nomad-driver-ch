...
func (h *taskHandle) DockerStatsToTaskResourceUsage(s *types.StatsJSON) *drivers.TaskResourceUsage {
	...
	h.logger.Debug("stats calculator", "model_name", stats.CPUModelName(), "total_cpu", stats.CPUNumCores(), "mhz_per_core", stats.CPUMHzPerCore(), "total_ticks", hclog.Fmt("%+v", cs.TotalTicks), "total_ticks_available", hclog.Fmt("%f", stats.TotalTicksAvailable()))
